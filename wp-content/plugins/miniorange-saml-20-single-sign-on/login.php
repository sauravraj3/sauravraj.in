...
class saml_mo_login {
	...
	function plugin_settings_style( $page) {
        ...
        wp_enqueue_style( 'mo_saml_admin_settings_style', plugins_url( 'includes/css/style_settings.min.css', __FILE__ ), array(), '4.8.91', 'all' );
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			if ( self::mo_check_option_admin_referer("login_widget_saml_save_settings")) {
				...
				if ( $this->mo_saml_check_empty_or_null( $_POST['saml_identity_name'] ) || $this->mo_saml_check_empty_or_null( $_POST['saml_login_url'] ) || $this->mo_saml_check_empty_or_null( $_POST['saml_issuer'] ) ) {
					...
					$saml_identity_provider_identifier = sanitize_text_field(trim($_POST['mo_saml_identity_provider_identifier_name']));
			...
			if ( isset( $_POST['option'] ) and $_POST['option'] == "mo_saml_identity_provider_identifier_option" ) {
				if(isset($_POST['mo_saml_identity_provider_identifier_input'])){
					$saml_identity_provider_input = sanitize_text_field($_POST['mo_saml_identity_provider_identifier_input']);
					update_option('mo_saml_identity_provider_identifier',$saml_identity_provider_input); 
				}
				if(isset($_POST['mo_saml_identity_provider_identifier_name_input'])){
					$mo_saml_identity_provider_identifier_name = sanitize_text_field($_POST['mo_saml_identity_provider_identifier_name_input']);
					update_option('mo_saml_identity_provider_identifier_name',$mo_saml_identity_provider_identifier_name_input);
				}
			}
